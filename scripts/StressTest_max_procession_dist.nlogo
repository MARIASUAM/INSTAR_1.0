...
to setup
 ca ; clear-all
  ...
  ;; ====================================
  ; set execution_name remove ":" date-and-time
  set execution_name word remove ":" date-and-time max_procession_dist ;; for StressTests
  ;read_parameters "../inputs/parametros.txt"
  ;read_parameters "../inputs/parametros_noenddate.txt" ; for calibration purposes
  read_parameters "../inputs/parameters_StressTest_max_procession_dist.txt" ; for StressTest_max_procession_dist
...
to update_colonies
  ...
  ask colonies [
    ...
    if (days_as_L2 <= 0) and (instar = "L2") [
      ...
      let pot_neighbours patches in-radius max_procession_dist ; for StressTest_max_procession_dist

      ;let best_neighbour min-one-of pot_neighbours [ distance myself ] ; disabled for StressTest_max_procession_dist
      let best_neighbour one-of pot_neighbours ; for StressTest_max_procession_dist
...
to generate_emergence_map
  gis:store-dataset gis:patch-dataset emergence_prob (word "../outputs/StressTest_max_procession_dist_" "emergence_prob.asc")
end
@#$#@#$#@
GRAPHICS-WINDOW
9
